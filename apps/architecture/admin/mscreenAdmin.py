...
class mscreenResource(resources.ModelResource):
...
class serviceFilter(admin.SimpleListFilter):
    ...
    def bussiness_id(self):
        rs = bussiness_type.objects.get(name='mscreen')
        return rs.id
...
@admin.register(mscreen)
class mscreenAdmin(ImportExportModelAdmin):
    resource_class = mscreenResource
