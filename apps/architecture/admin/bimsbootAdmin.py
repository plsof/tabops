...
class bimsbootResource(resources.ModelResource):
...
class serviceFilter(admin.SimpleListFilter):
    ...
    def bussiness_id(self):
        rs = bussiness_type.objects.get(name='bimsboot')
        return rs.id
...
@admin.register(bimsboot)
class bimsbootAdmin(ImportExportModelAdmin):
    resource_class = bimsbootResource
